...
def generate_filename(endpoint: str, directory: str = ".") -> str:
    ...
    files = [f for f in os.listdir(directory) if f.startswith(base_name) and f.endswith(".json")]
    if files:
        numbers = [int(f[len(base_name)+1:-5]) for f in files if f[len(base_name)+1:-5].isdigit()]
